...
def proof_records():
    ...
    :return:
    """
    eval_fields = [g.db_header[i - 1] for i in g.sample_fields]
...
def sql_proof_records():

    db = sqlite3.connect('split.db')
    print(len(g.sample_rowid))

    # rslt = db.execute(("SELECT *, '' AS r FROM records "
    #                    "AS a WHERE ROWID IN ({0}) UNION ALL "
    #                    "SELECT *, random() AS r FROM records "
    #                    "AS b WHERE ROWID NOT IN ({0}) ORDER "
    #                    "BY r DESC LIMIT {1}};".format('', limit)))
    #
    # print(rslt.fetchall())
...
if __name__ == '__main__':
    #TODO program to read just headers
    global g
    g = GlobalVarTest()
    g.ask_questions()
    # print(g.__dict__)
